...
/*!
 * @brief AD56x Command Definitions
 */
typedef enum {
    AD56x_CMD_NO_OP                         = 0x00, /* No operation */
    AD56x_CMD_W_INPUT_REG_N                 = 0x01, /* Write to Input Register n (Dependent on !LDAC) */
    AD56x_CMD_UPDATE_DAC_FROM_INPUT_REG_N   = 0x02, /* Update DAC Register n with contents from Input Register n */
    AD56x_CMD_WRITE_DAC                     = 0x03, /* Write to and update DAC Channel n */
    AD56x_CMD_POWER_DAC                     = 0x04, /* Power down/power up DAC */
    AD56x_CMD_HW_LDAC_MASK                  = 0x05, /* Hardware !LDAC mask register */
    AD56x_CMD_SOFT_RESET                    = 0x06, /* Software reset (power-on reset) */
    AD56x_CMD_INT_REF_SETUP                 = 0x07, /* Internal reference setup register */
    AD56x_CMD__MAX__
} ad56x_cmd_t;
...
ad56x_return_code_t ad56x_writeChannel(ad56x_dev_t *dev, const ad56x_output_channel_t ch, const uint16_t outputVal) {
    ad56x_dac_register_t dac_register = {0};

    if( (dev == NULL) || (dev->intf.write == NULL) ) {
        return AD56x_RET_NULL_PTR;
    }
    else if( (ch >= AD56x_OUTPUT_CH__MAX__) || (dev->intf.i2c_addr > 0x7F) ) {
...
ad56x_return_code_t ad56x_setOperatingMode(ad56x_dev_t *dev, const ad56x_output_channel_t ch, const ad56x_operation_mode_t mode) {
    ad56x_operation_register_t op_register = {0};
    ...
    switch( ch ) {
        case AD56x_OUTPUT_CH_A:
            dev->registers.bits.CHA_mode = mode;
            break;

        case AD56x_OUTPUT_CH_B:
            dev->registers.bits.CHB_mode = mode;
            break;

        case AD56x_OUTPUT_CH_A_B:
            dev->registers.bits.CHA_mode = mode;
            dev->registers.bits.CHB_mode = mode;
            break;
    ...
    op_register.bits.command.bits.cmd = AD56x_CMD_POWER_DAC;
